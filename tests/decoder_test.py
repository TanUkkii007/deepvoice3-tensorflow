...
@composite
def memory_tensor(draw, batch_size, input_length=integers(5, 50),
                  embed_dim=integers(4, 30).filter(even_number), elements=integers(-5, 5)):
...
@composite
def all_args(draw, batch_size=integers(1, 3), query_channels=integers(2, 256).filter(even_number), in_dim=integers(2, 80), r=integers(1, 1)):
    bs = draw(batch_size)
    _in_dim = draw(in_dim)
    _r = draw(r)
...
class DecoderTest(tf.test.TestCase):
    ...
    @given(args=all_args(),
           max_positions=integers(10, 50),
           preattention_out_features=integers(2, 20).filter(even_number), num_preattention=integers(1, 1),
           num_mha=integers(1, 4))
    @settings(max_examples=10, timeout=unlimited)
    def test_decoder(self, args,
                     max_positions,
                     preattention_out_features, num_preattention, num_mha):
        ...
        preattention_args = ((preattention_in_features, preattention_out_features),) * num_preattention
        ...
        decoder_online = Decoder(embed_dim, in_dim, r, max_positions, preattention=preattention_args,
                          mh_attentions=(mha_arg,) * num_mha,
                          dropout=1.0, is_incremental=True)
        ...
        out = decoder((keys, values), input=tf.constant(query),
